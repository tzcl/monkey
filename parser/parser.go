...
// NOTE: Haskell implements precedence by giving operators one of 10 precedence
// levels
const (
	_ int = iota
	LOWEST
...
func (p *Parser) parseBoolean() ast.Expression {
	b := &ast.Boolean{Token: p.currToken}
...
func (p *Parser) parseExpressionStatement() *ast.ExpressionStatement {
	...
	// if there is a semicolon, swallow it up
	if p.peekTokenIs(token.SEMICOLON) {
		p.nextToken()
	}
...
// NOTE: the heart of our Pratt parser
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	// Finally, we compare precedence (think 'stickiness').
	//
	// If the current precedence is higher than the next precedence, we stop
	// recursing and return the current expression.
	//
	// Otherwise, we keep building up the expression (suck in the RHS).
